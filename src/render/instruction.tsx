...
import { InstructionData } from "../route/types";
...
export const Instruction: React.FunctionComponent<InstructionProps> = ({
	icon, text, comment,indicatorClass,isSectionTitle,lineNumber,unindentStep,stepNumber,counterClassName,counterNumber,detail,detailRowSpan,indentIcon,image,imageRowSpan,directionMode,variables
})=>{
	if(isSectionTitle){
	...
	if(icon){
		...
		return <>
			<tr>
				...
				{detailRowSpan && <td className={clsx("detail",detail&&"detail-box")} rowSpan={detailRowSpan}>{detail&&<TextRender textBlock={detail} directionMode={directionMode} variables={variables}/>}</td>}
