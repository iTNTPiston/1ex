...
export const createLiveSplitFile = (instructions: InstructionData[]): string => {
	...
	return `${header}${createSegmentTags(instructions)}${footer}`;
};

const createSegmentTags = (instructions: InstructionData[]): string => {
	const splitNames: string[] = [];
	const splitIcons: string[] = [];
	instructions.forEach((data,i)=>{
